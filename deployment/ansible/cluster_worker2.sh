...
echo "== Setup cluster =="
curl -X POST -H "Content-Type: application/json" http://admin:admin@172.26.132.216:5984/_cluster_setup  -d '{"action":"enable_cluster", "bind_address":"0.0.0.0", "username":"admin", "password":"admin", "port":"5984", "remote_node":"172.26.134.61", "node_count":"3", "remote_current_user":"admin", "remote_current_password":"admin"}'

echo "== Add node =="
curl -X POST -H "Content-Type: application/json" http://admin:admin@172.26.132.216:5984/_cluster_setup  -d '{"action":"add_node", "host":"172.26.134.61", "username":"admin", "password":"admin", "port":"5984"}'
